...
def taxonomy_from_results_json(f):
    """
    Load a Taxonomy from a One Codex results.json file

    Args:
        f (str, file): A filepath or file handle pointing to a
                       results.json file generated by a classification job

    Returns:
        Taxonomy
    """
    tax_graph = networkx.DiGraph()
    def add_tax_record(t):
        tax_id = str(t['id'])
        attr_dict = {
            'name': t['name'],
            'rank': t.get('rank', 'no rank'),
        }
        tax_graph.add_node(tax_id, attr_dict=attr_dict)
        for child in t.get('children', []):
            add_tax_record(child)
            tax_graph.add_edge(str(child['id']), tax_id)

    input_json = read_json(f).get('taxonomy')
    add_tax_record(input_json)
